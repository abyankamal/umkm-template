<div class="rounded overflow-hidden shadow-lg bg-white">
    <script>
        document.title = 'Manajemen Artikel';
    </script>
    ...
    <div class="container px-6 mx-auto">
        ...
        <div class="flex justify-between items-center">
            ...
            <button wire:click="resetFields" class="bg-kutamis-purple text-white px-4 py-2 rounded-lg hover:bg-kutamis-purple-hover">
                Tambah Artikel Baru
            </button>
        </div>
        ...
        <div class="mb-4 grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
                <input wire:model.live="searchTerm" type="text" placeholder="Cari Artikel..."
                ...
                <table class="w-full whitespace-no-wrap">
                    <thead>
                        <tr class="text-xs font-semibold tracking-wide text-left text-gray-500 uppercase border-b bg-gray-50">
                            ...
                            <th class="px-4 py-3 cursor-pointer">Judul Artikel</th>
                    ...
                    <tbody class="bg-white divide-y">
                        @if($articles->isEmpty())
                        <tr>
                            <td colspan="4" class="text-center px-4 py-3 text-gray-500">
                                Belum Ada Artikel
                            </td>
                        </tr>
                        @else
                        ...
                        <tr class="text-gray-700">
                            ...
                            <td class="px-4 py-3">{{ Str::limit($article->content, 50) }}</td>
                            <td class="px-4 py-3">
    ...
    <div class="fixed inset-0 z-30 flex items-center justify-center overflow-auto bg-black bg-opacity-50" x-show="$wire.showModal"
        ...
        <div class="relative px-6 py-4 bg-white rounded-lg shadow-lg max-w-md w-full mx-4" @click.away="$wire.showModal = false">
            ...
            <form wire:submit="{{ $articleId ? 'updateArticle' : 'createArticle' }}">
                <div class="space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Judul Artikel</label>
                        <input type="text" wire:model.live="title"
                            class="w-full px-2 py-1 border rounded-lg focus:outline-none focus:border-kutamis-purple" placeholder="Masukan Judul Artikel">
                        @error('title') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700">Konten</label>
                        <textarea wire:model.live="content" rows="5"
                            class="w-full px-2 py-1 border rounded-lg focus:outline-none focus:border-kutamis-purple" placeholder="Masukan Konten Artikel"></textarea>
    ...
    <div x-data="{ show: false, message: '' }"
        x-on:article-saved.window="show = true; message = $event.detail; setTimeout(() => show = false, 3000)"
        x-on:article-deleted.window="show = true; message = $event.detail; setTimeout(() => show = false, 3000)"
    ...
    <div class="fixed inset-0 z-30 flex items-center justify-center overflow-auto bg-black bg-opacity-50" x-show="$wire.showDeleteModal"
        ...
        <div class="relative px-6 py-4 bg-white rounded-lg shadow-lg max-w-md w-full mx-4" @click.away="$wire.showDeleteModal = false">
            ...
            <p class="text-gray-700 mb-4">Apakah Anda yakin ingin menghapus artikel ini?</p>
            <div class="mt-5 flex justify-end space-x-3">
